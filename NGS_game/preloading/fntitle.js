//title card
class fntitle extends Phaser.Scene{
    constructor(){
        super({key:"fntitle"});
    }
    preload(){
        this.load.image('fntc','Fragmentation Title Card.png');
        this.load.audio("bgmusic",'../fruitninja/assets/bensound-betterdays.mp3');
    }
    create(){
        this.image=this.add.image(window.innerWidth/2,300,'fntc');
      ...
      clickButton5.on('pointerdown', () =>{this.scene.start("Fragmentation_intro");bgmusic.stop()});
