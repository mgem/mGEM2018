class Assembly_intro extends Phaser.Scene{
    constructor(){
        super({key:"Assembly_intro"});
    }
    preload(){
        this.load.image('Assem_intro','Assembly Loading GIF.gif');
        this.load.audio("bgmusic",'../fruitninja/assets/bensound-betterdays.mp3');
    }
    create(){
        this.image=this.add.image(window.innerWidth/2,300,'Assem_intro');
